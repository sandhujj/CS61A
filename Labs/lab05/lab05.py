...
def my_map(fn, seq):
    ...
    [None, None, None]
    """
    return [fn(seq[x]) for x in range(len(seq))]
...
def my_reduce(combiner, seq):
    ...
    cumulation = seq[0]
    for x in seq[1:]:
        cumulation = combiner(cumulation, x)
    return cumulation
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    new_city = make_city("bruh", lat, lon)
    if distance(new_city, city_b) < distance(new_city, city_a):
