...
def insert_items(lst, entry, elem):
    ...
    for index in range(len(lst)):
        if adder + index > len(lst) - 1: 
            break
        elif lst[index + adder] == entry == elem:
            lst.insert(index + adder + 1, elem)
            adder += 2
        elif lst[index + adder] == entry:
            lst.insert(index + adder + 1, elem)
            adder+=1
    if lst[len(lst) - 1] == entry: 
        lst.append(elem)
    return lst
...
def repeated(t, k):
    ...
    # prev_val = next(t)
    # count = 1
    # for val in range(sum(1 for e in t) - 2):
    #     curr_val = next(t)
