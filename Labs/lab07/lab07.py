...
class Person:
    ...
    def __init__(self, name):
        self.name = name
        self.last_said = 'I squirreled it away before it could catch on fire.'
    ...
    def ask(self, stuff):
        self.last_said = "Would you please " + stuff
        return self.say("Would you please " + stuff)

    def greet(self):
        self.last_said = "Hello, my name is " + self.name
...
class SmartFridge:
    ...
    def use_item(self, item, quantity):
        "*** YOUR CODE HERE ***"
        bruv = self.items[item] - quantity
        if bruv <=0:
            self.items[item] = 0
            return f'Oh no, we need more {item}!'
        else:
            self.items[item] = bruv
            return f'I have {bruv} {item} left'
