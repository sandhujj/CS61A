    ...
    def nearest_bee(self):
        ...
        while True:
            if jaunt.is_hive:
                ...
                if jaunt.bees and self.lower_bound <= x <= self.upper_bound:
                    x+=1
                    return random_bee(jaunt.bees)
                else:
                    x+=1
                    jaunt = jaunt.entrance
    ...
    def action(self, gamestate):
        if self.turns_to_chew:
            ...
            if self.place.bees:
                ...
                victim = random.choice(self.place.bees)
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate): 
        ringmuhs = self.place.bees[:]
        for beems in ringmuhs:
            beems.reduce_health(self.damage)
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while jaunt.exit:
            ...
            if jaunt.ant:
                jaunt.ant.double()
                if jaunt.ant.is_container:
                    if jaunt.ant.ant_contained:
                        jaunt.ant.ant_contained.double()
