...
def next_perfect_square(current_square): 
    for num in range(current_square + 1):
        if num * num == current_square: 
            return (num + 1) * (num + 1)
    return False
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    curr_score = strategy(0, 0)
    for score in range(100):
        for opponent_score in range(100): 
            if strategy(score, opponent_score) != curr_score: 
                return False
    return True
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    for num in range(1, 11): 
        curr = make_averaged(roll_dice, total_samples)(num, dice)
        if curr > max: 
            max = curr
            ret = num
    return ret
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    if tail_points(opponent_score) >= threshold or perfect_square(tail_points(opponent_score) + score): 
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
